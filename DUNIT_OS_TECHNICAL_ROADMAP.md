...
- [ ] Оставить старый kernel GUI под `legacy_gui` feature до parity, затем удалить. Feature-гейт готов: `gui`/`ui_loop`/`window_manager` и их вызовы (`window_manager::init`, `ui_loop::run_ui_loop`, `gui_terminal_write_exec_output`) под `#[cfg(feature = "legacy_gui")]`, feature включён в `default` — boot к встроенному десктопу без изменений (проверено: gui_server + два клиента OK, exit 0, VFS clean). При выключенном feature GUI-ветка уходит в headless hlt с сообщением, что десктоп обслуживает userspace gui_server. Остаётся: достичь parity в userspace-компоситоре, затем снять feature и удалить kernel-GUI.
