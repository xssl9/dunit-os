...
- [~] Оставить старый kernel GUI под `legacy_gui` feature до parity, затем удалить. Feature-гейт готов: `gui`/`ui_loop`/`window_manager` и их вызовы под `#[cfg(feature = "legacy_gui")]`, feature в `default` — boot к встроенному десктопу без изменений. При выключенном feature (`--no-default-features`, конфиг `limine_dwm.conf`) GUI-ветка ядра теперь **автозапускает `/app/gui_server` как десктоп** через `run_foreground_exec` (не headless hlt) — загрузка идёт прямо в userspace-компоситор. Остаётся: достичь parity в userspace, затем снять feature и удалить kernel-GUI.
